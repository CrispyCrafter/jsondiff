...
from ujsondiff import JsonDiffer, add, delete, diff, discard, insert, replace
...
class JsonDiffTests(unittest.TestCase):
    def test_a(self):
        ...
        self.assertEqual({3, 4}, diff({1, 2}, {3, 4}))
        self.assertEqual(
            {replace: {"c": 3, "d": 4}}, diff({"a": 1, "b": 2}, {"c": 3, "d": 4})
        )
        ...
        self.assertEqual(
            {delete: ["b"], "c": 3}, diff({"a": 1, "b": 2}, {"a": 1, "c": 3})
        )
